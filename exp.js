...
minSetSize = 1; //4 // starting length of each trial (i.e., min number of letters in a trial)
maxSetSize = 1; //7 // ending length of each trial (i.e., max number of letters in a trial)
repSet = 1; //3 // number of times each set size should be repeated
...
var cogloadf = function (correct) {
  ...
  operation = jsPsych.randomization.sampleWithReplacement(
    possibleOperations,
    1
  )[0];
  if (operation == " + ") {
...
var instructions = {
  ...
  pages: function () {
    pageOne =
      '<div style="font-size:20px;"><b>INSTRUCTIONS</b><br><br><br>This is the operation span task. <br><br>This task has two parts: <br>(1) Letter memorization <br>(2) Math problem solving<br><br><br></div>';
    pageTwo =
...
var instructions4 = {
  ...
  pages: function () {
    pageOne =
      '<div style="font-size:20px;">We have finished with the practice trials.<br><br><b>We will now start with the main trials.</b><br>Proceed only if you have understood the task.<br><br>Press "Next" to start the main trials.<br><br></div>';
    return [pageOne];
  },
  allow_backward: false,
  button_label_next: "Next",
...
var cog_load_demo = {
  ...
  equation_accuracy: function () {
    ...
    if (nCogLoadDemo == 1) {
      ...
      eqCorrect = jsPsych.randomization.sampleWithReplacement(
        [true, false],
        1
      )[0];
    }
...
var feedback = {
  ...
  pages: function () {
    pageOne =
      ...
      " out of " +
      nLetters +
      "</font> letters in their correct order.</b><br><br>";
    if (n > nPracticeTrials) {
      pageOne +=
        "You solved <font color='blue'>" +
        nMathAcc +
        " out of " +
        nLetters +
        "</font> math problems accurately.<br><br></div>";
    }
    return [pageOne];
  },
  allow_backward: false,
...
var feedbackLoad = {
  ...
  stimulus: function () {
    ...
    if (accuracy == 1) {
      text +=
        '<div style="font-size:35px; color:rgb(0 220 0)"><b>Correct<br><br><br><br></div>';
    } else {
      text +=
        '<div style="font-size:35px; color:rgb(240 0 0)"><b>Incorrect<br><br><br><br></div>';
    }
...
var dataLog = {
  ...
  on_finish: function () {
    var data = jsPsych.data
      .get()
      .filter([
        { trial_type: "operation-span-recall" },
        { trial_type: "html-button-operationspan" },
      ]);

    // Convert data to JSON or a compact string
    var jsonData = JSON.stringify(data.values());

    // Save to Qualtrics Embedded Data
    Qualtrics.SurveyEngine.setEmbeddedData("WM_Task_Data", jsonData);
  },
};
